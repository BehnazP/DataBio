...
%
% [P,ff,rho,omega2,lnR,sumX,logDet] = wishart_change_Rj(X,noL,pol);

% Input
%   X       - 4-D array series of k 1x1, 2x2 or 3x3 complex covariance matrices
...
%                                            vvvv (real)
%   noL     - k (or one) number of looks for X
%   pol     - polarization, 9='full', 5='azim', 4='dual', 3='fdiag', 2='ddiag' or 1='single'
%
% Output
%   lnR     - logarithm of test quantities Q and Rj
%             ln(Q) is lnR(:,:,1), ln(R_2) in the article is lnR(:,:,2) etc.
...
%
% Note: lnQ is the sum of the lnR's, i.e., lnQ = sum(lnR(:,:,2:end),3)
%
% Reference
%
...
% http://orbit.dtu.dk/en/publications/determining-the-points-of-change-in-time-series-of-polarimetric-sar-data(4255ddb4-1fd9-4d66-9aad-e8049a976d75).html
...
if nargin>3
    error('wishart_change_Rj: wrong input');
end
if nargout>7
    error('wishart_change_Rj: wrong output');
end
