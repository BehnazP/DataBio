...
% First version 11.01.2018
% minor changes to make the code faster
...
if pflag==1
    ...
    elapsed_time = ['CPU time used for EM_GM: %5.2fs',cputime-t];
    disp(elapsed_time);    
    disp(['Number of iterations: %d',niter-1]);
end
%%%%%%%%%%%%%%%%%%%%%%
%%%% End of EM_GM %%%%
%%%%%%%%%%%%%%%%%%%%%%
...
iV = zeros(d,d,k);
for j=1:k
    %%%Bezo what is the value of eps? need to be define
...
L = 0;
for i=1:k
    iV = (V(:,:,i)); % is this the inverse? no u are using \ so it actually is same here!
    %%%% Bezo iV == V(:,:,i)%% %
